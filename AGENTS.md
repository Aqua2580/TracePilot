...
17. **当前 Phase 4 尚未验收通过。** 修改 Phase 4 相关代码前，必须阅读 `docs/reviews/PHASE-4-ACCEPTANCE-REVIEW.md`，逐项关闭其中 P1 并补齐指定的单元、集成、API 对抗性和真实运行时测试；不得进入 Phase 5。尤其不得以 prompt 或 `--auto-approve` 绕过命令/路径/审批策略，不得向 Agent 可执行的验证进程泄漏凭据，也不得信任调用方提交的 Diff 或验证结果。
